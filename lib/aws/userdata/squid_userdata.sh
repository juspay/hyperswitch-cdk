...
# Setup Wazuh
aws s3 cp {{bucket_name}}t/wazuh.conf /var/ossec/etc/ossec.conf
chown root:ossec /var/ossec/etc/ossec.conf
systemctl restart wazuh-agent

# Setup Vector
rm /etc/vector/vector.toml
aws s3 cp {{bucket_name}}/squid_vector.toml /etc/vector/vector.toml
chown vector:vector /etc/vector/vector.toml
systemctl restart vector

# update squid conf
cp /etc/squid/squid.conf /etc/squid/squid.conf.old
aws s3 cp {{bucket_name}}/squid.conf /etc/squid/squid.conf

# Refresh squid
echo 'aws s3 cp {{bucket_name}}/whitelist.txt /etc/squid/tmp_whitelist.txt
